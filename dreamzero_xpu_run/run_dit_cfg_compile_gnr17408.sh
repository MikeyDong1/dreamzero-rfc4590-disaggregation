#!/usr/bin/env bash
# Orchestrate the CFG-off + inductor DiT-only DreamZero replay on gnr17408 card0,
# inside the vllm 0.24 / torch 2.12 image. WARM clean sweep (TTC) + one profiled
# N=16 run. Same K as the eager+CFG baseline.
#
# Usage: run_dit_cfg_compile_gnr17408.sh <run_id> <K> <cfg on|off> <compile eager|inductor> <profile_steps> [image] [container]
set -Eeuo pipefail
...
COMPILE="${4:-inductor}"
PSTEPS="${5:-16}"
IMAGE="${6:-vllm-omni-xpu:latest}"
CONTAINER="${7:-vllm-omni-cc-mikey-gnr17408-024}"
...
# inductor cache MUST live on the big /data disk (/ is only 23G); reuse across runs.
